...
- **How I reproduced it:** The two existing tests in `tests/test_playlists.py` already reproduce this. `test_playlist_returns_all_songs` seeds a playlist with 5 songs and asserts `get_playlist_songs` returns 5, but it returned 4 (`assert 4 == 5`). `test_playlist_returns_songs_in_order` asserts the titles are `Track 1`…`Track 5`, but the returned list stopped at `Track 4`. Both point at the same symptom: the final song is missing.

- **How I found the root cause (navigation path):** I opened `playlist_service.py` and read `get_playlist_songs`. The database query is correct — it joins through `playlist_entries`, filters by the playlist, and orders by `position` ascending, then calls `.all()`, which returns every song. I then looked at the very last line, the return statement: `return [song.to_dict() for song in songs[:-1]]`. The `[:-1]` slice was the giveaway — the query fetches all the songs, but this slice throws one away before returning.

- **The root cause:** In Python, the slice `songs[:-1]` means "every item except the last one." So even though the query correctly retrieved all songs in position order, the return statement dropped the final song. Because songs are ordered by `position`, the item removed was always the one with the highest position — i.e., the last song in the playlist — which is exactly the reported behavior.
